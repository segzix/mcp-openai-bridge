...
export function printInteractiveHelp(commandName) {
  console.log(`
${commandName} 使用方式：
单次模式：
  ${commandName} -C /path/to/project 请分析这个项目
  ${commandName} --workdir /path/to/project 请列出文件
多轮终端模式：
  ${commandName}
  ${commandName} -C /path/to/project
终端内命令：
  /help               显示帮助
  /pwd                显示当前工作目录
...
export async function runSingleQuestion({ question, workdir }) {
...
export async function runInteractiveShell({ commandName, workdir }) {
  ...
  console.log(`${commandName} 多轮终端模式`);
  ...
  try {
    while (true) {
      const line = (await rl.question(`\n${commandName}:${currentWorkdir}$ `)).trim();
      ...
      if (line === "/help") {
        printInteractiveHelp(commandName);
        continue;
      }
...
export async function runCli({ commandName, argv }) {
  const { question, workdir } = parseCliArgs(argv);

  if (question) {
    await runSingleQuestion({ question, workdir });
    return;
  }

  await runInteractiveShell({ commandName, workdir });
}
